...
export class Hud {
    ...
    constructor(scene) {
        this.scene = scene;
        let shoppingCenter = new Phaser.GameObjects.Sprite(scene,100,100, 'plates','shoppingCenter')
        let house = new Phaser.GameObjects.Sprite(scene,200,100, 'plates','house')
        let church = new Phaser.GameObjects.Sprite(scene,300,100, 'plates','church')
        let groceryStore = new Phaser.GameObjects.Sprite(scene,400,100, 'plates','groceryStore')
        let roadStraight = new Phaser.GameObjects.Sprite(scene,500,100, 'plates','roadStraight')
        let roadExit = new Phaser.GameObjects.Sprite(scene,600,100, 'plates','roadExit')
        let roadCross = new Phaser.GameObjects.Sprite(scene,700,100, 'plates','roadCross')
        let roadExitDouble = new Phaser.GameObjects.Sprite(scene,800,100, 'plates','roadExitDouble')
        let roadCrossDouble = new Phaser.GameObjects.Sprite(scene,900,100, 'plates','roadCrossDouble')
        let roadCurve = new Phaser.GameObjects.Sprite(scene,1000,100, 'plates','roadCurve')
...
/*
        ...
        scene.input.on('pointerdown', (pointer) => {
            ...
            scene.input.activePointer.isDown = false;
                    let clickedNewPlate = new CustomEvent('clickedNewPlate', {
                        detail: {
                          name: x.frame.name
                        }});
                    dispatchEvent(clickedNewPlate);
                    // console.log(y);
                }
        ...
        //scene.add.existing(plate.setScale(0.5));    
    }
